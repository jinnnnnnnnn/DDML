...
parser = argparse.ArgumentParser(description=
    'Official implementation of `Deep Disentangled Metric Learning`'  
    + 'Our code is modified from `https://github.com/tjddus9597/Proxy-Anchor-CVPR2020.git`'
)
...
parser.add_argument('--disent', default = False, type = bool,
    help = 'Attach the DDML module'
)
parser.add_argument('--al', default = 0.0, type = float,
    help = 'DDML hyperparameter alpha setting'
)
parser.add_argument('--be', default = 0.0, type = float,
    help = 'DDML hyperparameter beta setting'
)
parser.add_argument('--gam', default = 0.0, type = float,
    help = 'DDML hyperparameter gam setting'
)
...
# Directory for Log
LOG_DIR = args.LOG_DIR + '/logs_{}/{}_{}_embedding{}_al{}_be{}_gam{}_{}_lr{}_batch{}{}'.format(args.dataset, args.model, args.loss, args.sz_embedding, args.al, 
                                                                                            args.be,  args.gam, args.optimizer, args.lr, args.sz_batch, args.remark)


# Wandb Initialization
wandb.init(project=args.dataset + '_DDML+PA', notes=LOG_DIR)
...
if args.loss == 'Proxy_Anchor':
    criterion = losses.Proxy_Anchor(nb_classes = nb_classes, sz_embed = args.sz_embedding, mrg = args.mrg, alpha = args.alpha, al=args.al, be=args.be, gam=args.gam).cuda()

# Train Parameters
param_groups = [
    {'params': list(set(model.parameters()).difference(set(list(model.model.embedding.parameters())+list(model.model.embedding_specific.parameters())))) if args.gpu_id != -1 else 
                 list(set(model.module.parameters()).difference(set(list(model.module.model.embedding.parameters())+list(model.module.model.embedding_specific.parameters()))))},
    {'params': model.model.embedding.parameters() if args.gpu_id != -1 else model.module.model.embedding.parameters(), 'lr':float(args.lr) * 1},
    {'params': model.model.embedding_specific.parameters() if args.gpu_id != -1 else model.module.model.embedding_specific.parameters(), 'lr':float(args.lr) * 1},
]
if args.loss == 'Proxy_Anchor':
...
if os.path.exists('{}'.format(LOG_DIR)):
    best_pth = '{}/{}_{}_best.pth'.format(LOG_DIR, args.dataset, args.model)
    load_state = torch.load(best_pth)
    model.load_state_dict(load_state['model_state_dict'])
    print('\n\n\tResume pretrained models\n\n')
...
for epoch in range(0, args.nb_epochs):
    ...
    if args.warm > 0:
        if args.gpu_id != -1:
            unfreeze_model_param = list(model.model.embedding.parameters()) +list(model.model.embedding_specific.parameters())+ list(criterion.parameters())
        else:
            unfreeze_model_param = list(model.module.model.embedding.parameters()) +list(model.module.model.embedding_specific.parameters()) +list(criterion.parameters())
    ...
    for batch_idx, (x, y) in pbar:
        if args.disent :
                output_emb, output_emb_spcf = model(x.squeeze().cuda())
                loss , logit, logit_spcf, loss_AGReg = criterion(output_emb, output_emb_spcf, y.squeeze().cuda(), args.disent)                         
